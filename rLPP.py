...
def rLLP(data,d):
    ...
    for num in range(N_MAX):#进行迭代
        if f(X,S,W,p) < f_best:#迭代后目标函数值减小
            ...
            for i in range(n):
                for j in range(n):
                    St[i, j] = S[i, j] * math.pow(np.linalg.norm( np.dot(W.transpose(), X[:, i].reshape(X.shape[0], 1)) - np.dot(W.transpose(),X[:, j].reshape(X.shape[0], 1)),ord=2), p)  # 更新相似度矩阵值
                Dt[i, i] = sum(St[i, :])  # 更新度矩阵值
            Lt = Dt - St #计算新的拉普拉斯矩阵
            tempsdata = np.dot(np.linalg.pinv(np.dot(np.dot(X,D),X.transpose())), np.dot(X, np.dot(Lt,X.transpose())))
            vector_values, vectors = np.linalg.eig(tempsdata)#求解特征值与特征向量
            vector_values = np.real(vector_values)
            vectors = np.real(vectors)
            index = np.argsort(vector_values)#对特征值进行有小到大排序
            W = np.zeros((m,d))
            for k in range(1,d+1):#选取排序后第2个至第d+1个特征值对应的特征向量
                W[:,k-1] = vectors[:,int(index[k])]
